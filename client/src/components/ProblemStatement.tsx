      ...
      {!loading ? (
        <>
          <div className="absolute right-0 flex space-x-2">
            <button
              className="group p-1 rounded-sm border border-gray-400 hover:bg-gray-700"
              title="Edit Problem"
              onClick={() =>
            ...
            <button
              className="group p-1 rounded-sm border border-gray-400 hover:bg-red-400 hover:border-red-400"
          ...
          {problem?.input && (
            <div className="my-6">
              <h2 className="text-lg text-gray-800 mb-2">Input Format</h2>
              <MDEditor.Markdown source={problem?.input} />
            </div>
          )}
          {problem?.output && (
            <div className="my-6">
              <h2 className="text-lg text-gray-800 mb-2">Output Format</h2>
              <MDEditor.Markdown source={problem?.output} />
            </div>
          )}
          {problem?.constraints && (
            <div className="my-6">
              <h2 className="text-lg text-gray-800 mb-2">Constraints</h2>
              ...
              <div key={index}>
                <div className="my-6">
                  ...
                  <p className="whitespace-pre-wrap bg-slate-300/50 p-4 rounded font-mono text-lg relative">
                    {item.input}
                    <button
                      className="absolute top-0 right-0 font-mono text-xs bg-transparent text-black border border-black/40 p-1 px-2 rounded hover:text-black hover:bg-white font-bold"
                ...
                {item.explanation && (
                  <div className="my-6">
                    <h2 className="text-lg text-gray-800 mb-2">Explanation</h2>
